...
<html lang="en">
	...
	<body>
		...
		<div class="container">
			...
			<!-- Tab links -->
			<div class="tab">
				<button class="tablinks" onclick="openTab(event, 'New')" id="newMatchesTab">New Matches</button>
				<button class="tablinks" onclick="openTab(event, 'Saved')" id="storedMatchesTab">Stored Matches</button>
			</div>
					...
					<div id="New" class="tabcontent">
						...
						while($requestRow = mysqli_fetch_assoc($matchRequests)){
							...
							while($row = mysqli_fetch_assoc($matchList)){
								...
								echo '<section class="row col-sm-12 carShortInfo" data-toggle="modal" data-target="#mod'.$newModelNum.'">';
									echo "<article class='col-sm-6'>";
										...
										echo "</ul>";
									echo "</article>";
									echo '<aside class="col-sm-6" data-toggle="modal" data-target="#mod'.$newModelNum.'">';
										echo '<img class="carPhoto" src="data:image/jpeg;base64,'.base64_encode( $row['photo'] ).'" data-toggle="modal" data-target="#mod'.$newModelNum.'">';
									echo "</aside>";
								echo "</section>";
						
								// modal
								// some of the following code was sourced from: https://www.w3schools.com/bootstrap/bootstrap_modal.asp
									echo '<div class="modal fade" id="mod'.$newModelNum.'" role="dialog">';
		...
		var modelNum = '<?php echo $newModelNum; ?>';
		if(modelNum != 0) {
			document.getElementById("newMatchesTab").click();
		}
		else {
			document.getElementById("storedMatchesTab").click();
		}
